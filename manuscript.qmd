...
Beyond travel planning, generative AI is supporting core operations in tourism industries, including marketing, human resource management, and tourism experience design [@amadeus2024; @dogru2025].
...
Limited access to model internals and training data, combined with the black-box nature of generative AI, impedes revealing such biases [@bai2025; @gallegos2024]. 
...
They also produced more unevenly distributed tourist flows with stronger seasonality and less diversified demand.
...
Some preliminary works highlights benefits of generative AI to businesses and tourists.
...
In particular, social biases their ethical implications are prominent focus.
...
This trend aligns with broader literature on algorithmic biases that emphasize ethical challenges, espicially racism and stereotypes [see @ghosh2025; @kordzadeh2022]
Beyond ethical concerns, several schloars note risks of a popularity bias in generative AI, where algorithms favor popular options and underrepresent unpopular ones [@law2024; @lehto2025].
This bias poses practical concerns for the tourim sector, as it can intensify over-tourism at popular destinations while damaging tourism sectors at less popular destinations [@mellors2025].

Two key challenges complicate the analysis of generative AI biases: definition and measurement.
What constitutes "biased" algorithms is often vauge in tourism and hospitality studies.
...
Others distinguish bias from harm, defining bias as systematic inclination or deviation of an algorithm from empirical data [@ghosh2025; @wu2024a].
This netural definition of bias is closely tied to statistics and computer science that view bias as inherent and unavoidable [@chen2023d; @ghosh2025].
Thus, definition of algorithmic bias varies not only across disciplines but also by specific bias being examined.

Even with clear definitions, measuring biases in generative AI is difficult.
Despite attempts to make generative AI more transparent, many models remain proprietary and closed-source.
@ali2025 proposes using  Google’s What-If toolkits for assesing biases in AI-generated tourism data.
...
Bias detection is further complicated by safty tunings that recent generative AI models undergo to avoid explicit biases [@bai2025; @santurkar2023].
...
For example, metrics may reveal that generative AI disproportionately suggest resort destinations to certain racial groups.
...
Examples projection models include Shared Socioeconomic Pathways for climate trajectories and COVID-19 diffusion models [@adam2020; @ipcc2021].
...
Although unrealistic, such undilluted projections are necessary to reveal the full extent of their biases.
...
Our model capture such biases as negative rescaling factors, explainig algorithm outputs as mixtures of amplification and attenuation of empirical patterns.
...
Next, our simulations rely on empirically derived demographic profiles, instead of using arbitrary or non-representative profiles that would confound the results [for example, @andreev2025].
Third, we repeat simulation for 1,000 iterations to sufficiently capture the uncertainty in LLM outputs, unlike studies that relied on few handpicked responses or a single simulation run [for example @mellors2025; @xiong2024], 
...
This process resembles agent-based modeling using LLMs, but differs in that we prompt LLMs to act as travel agents not tourists [@gao2024].
The choice reflects our objective to project how generative AI would influence tourist flows if widely adopted, rather than using it to substitute for human travelers.
...
We use two different LLMs to establish the consistency of our findings across different models: Google's Gemini 2.5 Flash Lite (version June 2025) and OpenAI's GPT-4.1 Nano (version April 2025).
...
Gemini 2.5 family use mixture-of-experts architecture, while GPT-4.1 family rely on traditional transformer architecture.
Because the two models are developed by different companies, they are likely be trained on different datasets and tuning processes, which is ideal for assessing the generalizability of our findings.
...
By design, only about 3.2% (=1,000/31,212) of cells would have any tourist visits even under complete random assignment.
But the upper-tail of the distribution differs across simulations.
LLM simulations show stronger concentration of tourist in the most popular destination-origin-month combination than empricial-based ones (Max=`{r} max.gemini` for Gemini 2.5 Flash Lite and `{r} max.gpt` for GPT-4.1 Nano; `{r} max.advan` for ADVAN Mobility Data and `{r} max.nhts` for National Household Travel Survey).

Similar patterns emerge when examining presence and abscense of tourist flow.
...
About `{r} intext.any$only.nhts.gpt` to `{r} intext.any$only.advan.gemini` of empirically obaserved combinations never appeared in LLM simulations.
By contrast, LLMs rarely suggest destination-origin-month combinations that are absent from empirical data, with fewer than 10% of combinations only appear in LLM simulations.
...
We further examine differences in distribution of tourist flow across simulations.
...
Inequality is measured using Gini index, where higher values indicates greater concentration of tourists across states or months.
...
Gini indices of monthly tourist share for LLM simulations far exceeds empirical levels (Gemini 2.5 Flash Lite=`r month.gini.gemini` and GPT-4.1 Nano=`r month.gini.gpt`; ADVAN Mobility Data=`r month.gini.advan` and National Household Travel Survey=`r month.gini.nhts`).
Although, the two LLMs differ in peak tourism months.
Gemini 2.5 Flash Lite tend to recommend traveling in September and October, where as GPT-4.1 Nano shows peaks in April, May, and September.
LLMs worsen the seasonality of tourism demand for most destinations.
For @fig-prop[c](), we calculated Gini index of monthly tourist share for each destination state and took the median over 1,000 iterations.
In empirical-based simulations, few states show relatively high seasonality such as Alaska and Hawaii.
...
LLMs generate tourists flows that destinations rely on a few origins (@fig-prop[d]()).
We use entropy instead of Gini index because entropy is more suitable for 
measuring whether a destination have diversified and balanced tourism demand, which are characteristics of resilient destinations [@lee2025c]
...
LLMs produce tourist flows with clear separation between tourist sending and receving states, as indicated by weaker reciprocity.
Reciprocity measures the tendency of two states exchanging a similar number of tourists.
...
But the two models differ in tendency to suggest in-state tourism.
...
Biases in spatial perception of these models is one possible explanation, such as overestimation of inter-regional distance [@fulman2024].
...
For example, two GPT-4.1 models also show bias toward popular origin-destination pairs, while other LLMs attenuate empirically popular destinations.
Unfortunately, this study could not identify why LLMs show different types and degree of popularity biases.
...
This study provides early empirical evidence that generative AI poses significant potential to undermine the sustainability and resilience of the tourism sector.
...
We show that such biases have tangible consequencesfor destinations, including stronger seasonality, reduced demand diversity, and weaker reciprocal exchange of tourists.
All these outcomes defy conditions for fostering sustainable and resilient tourism sector [for example, @cisneros-martinez2018; @lee2025c; @wttc2022].
...
The existing methods often required access to internal model parameters and mainly tested precense of bias [@bai2025; @chen2023d].
...
While these explanations are still correlational than causal, they are basis for unraveling systemic mechanisms behind the black-box algorithms.

This study focused on measuring bias in genreative AI from supplier-side and its implications for destinations.
However, bias and their consequences can also be studied from demand-side, focusing on whether *tourists* receive personalized and diverse travel options.
For instance, recommendation algorithm with supplier-side popularity bias can lead to users receiving less satisfying options [@abdollahpouri2020].
Similarly, casues of these biases can also be spectulated from both demand- and supply-side.
Biases may also arise from both learning demand-side popularities and supply-side imbalances visibility of destenations in training data.

Our LLM simulations are projections wherein all tourists’ decisions are made by generative AI.
This hypothetical scenario allowed us to anticipate consequences of generativi AI adoption rather than document them *a posteriori*.
...
Consider following limitations when interpreting our findings.
...
Two individuals with identical demographic profile may visit the same destination with entirely different motivations.
...
We also do not account for interactions between simulated individuals, although empirical studies showone tourist's decision influencing decisions of others [@lee2025a].
Further, studies show that generative AI exhibits stonger bias in relative decisions than absolute ones [@bai2025].
Because we intructed LLMs to recommend any single destination instead of giving alternatives, the degree of popularity bias shown in this study may be conservative.
...
Future research could refine how popularity biases are measured, including non-linear effects where destinations with popularity under certain threshold receive no recommendations at all.
Our operationaliztion of the Baseline-Rescaling-Outcome Model is also less reliable when the peaks of empirical and generative AI-driven tourism patterns poorly align.
...
The authors used GPT-5.2 for grammar and style editing of this manuscript.
The manuscript were carefully reviewed and edited by the authors to ensure accuracy of content.
