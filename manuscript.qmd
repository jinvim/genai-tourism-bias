...
We instructed large language models to act as travel agents and suggest one domestic travel destination based on the provided demographic profile.
...
@tbl-any compares which destination-origin-month combinations are present or absent in large language model versus empirical simulations.
We aggregated the simulated visits across all 1,000 iterations to reduce the sparsity.
In all four comparisons, large language models exclude many destination-origin-month combinations.
...
We also observe that fewer states exhibit relatively high entropy values in the large language model simulations.
Such demand characteristics indicate that large language model produce a less resilient tourism sector, as less diversity of tourist origins and higher reliance on a few origin markets undermines resilience to shocks [@lee2025c].
